...
export function CampaignRow({ campaign }: { campaign: Campaign }) {
  ...
  return (
    <>
      ...
      >
        {/* Name + all badges + action tag — all in first column */}
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {/* Action tag — prominently in the name row */}
              ...
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                ...
                <span className="text-[10px] t3">{campaign.adSets.length} ad sets · {formatCurrency(campaign.dailyBudget)}/day</span>
              </div>
            </div>
          </div>
        </td>
        ...
        <td className="px-3 py-3 text-right">
          <p className={clsx(
            'text-sm font-bold',
            campaign.roas >= 4.5 ? 'text-emerald-600 dark:text-emerald-400' : campaign.roas >= 3.5 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'
          )}>
      ...
      {/* Ad sets expand beneath — with left-border accent for hierarchy */}
